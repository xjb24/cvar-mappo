...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        self.use_cvar = getattr(args, "use_cvar", False)
        self.num_users = getattr(args, "num_users", None)
        if self.use_cvar and (self.num_users is None or self.num_users <= 0):
            raise ValueError("启用 CVaR 约束时需要有效的 num_users。")
        ...
        self.rewards = np.zeros(
            (self.episode_length, self.n_rollout_threads, num_agents, 1), dtype=np.float32)
        # user_rates 始终存储（无论 use_cvar）
        self.user_rates = np.zeros(
            (self.episode_length, self.n_rollout_threads, self.num_users), dtype=np.float32)
        if self.use_cvar:
            self.cvar_values = np.zeros(
                (self.episode_length, self.n_rollout_threads, self.num_users), dtype=np.float32)
            self.cvar_betas = np.zeros(
                (self.episode_length, self.n_rollout_threads, 1), dtype=np.float32)
        else:
            self.cvar_values = None
            self.cvar_betas = None
    ...
    def insert(self, share_obs, obs, rnn_states_actor, rnn_states_critic, actions, action_log_probs,
               ...
               cvar_values=None, cvar_betas=None, user_rates=None):
        """
        ...
        if user_rates is not None:
            self.user_rates[self.step] = user_rates.copy()
        elif self.use_cvar and cvar_values is not None:
            # 兼容性：如果没有 user_rates 但有 cvar_values，使用 cvar_values
            self.user_rates[self.step] = cvar_values.copy()
        if self.use_cvar:
            if cvar_values is None or cvar_betas is None:
                raise ValueError("启用 CVaR 时需要提供 cvar_values 和 cvar_betas。")
            self.cvar_values[self.step] = cvar_values.copy()
            self.cvar_betas[self.step] = cvar_betas.copy()
    ...
    def feed_forward_generator_transformer(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        # user_rates 始终存在（无论 use_cvar）
        user_rates = np.repeat(self.user_rates[:, :, None, :], num_agents, axis=2).reshape(-1, self.num_users)

        if self.use_cvar:
            cvar_values = np.repeat(self.cvar_values[:, :, None, :], num_agents, axis=2).reshape(-1, self.num_users)
            cvar_betas = np.repeat(self.cvar_betas[:, :, None, :], num_agents, axis=2).reshape(-1, 1)
        ...
        for indices in sampler:
            ...
            if self.use_cvar:
                cvar_values_batch = cvar_values[indices].reshape(-1, self.num_users)
                cvar_betas_batch = cvar_betas[indices].reshape(-1, 1)
                user_rates_batch = user_rates[indices].reshape(-1, self.num_users)
                yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, \
                      value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, \
                      adv_targ, available_actions_batch, cvar_values_batch, cvar_betas_batch, user_rates_batch
            else:
                user_rates_batch = user_rates[indices].reshape(-1, self.num_users)
                yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, \
                      value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, \
                      adv_targ, available_actions_batch, user_rates_batch
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        for indices in sampler:
            ...
            if self.use_cvar:
                cvar_values_batch = cvar_values[indices]
                cvar_betas_batch = cvar_betas[indices]
                user_rates_batch = user_rates[indices]
                yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch,\
                      value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch,\
                      adv_targ, available_actions_batch, cvar_values_batch, cvar_betas_batch, user_rates_batch
            else:
    ...
    def naive_recurrent_generator(self, advantages, num_mini_batch):
        ...
        # user_rates 始终存在（无论 use_cvar）
        user_rates = np.repeat(self.user_rates[:, :, None, :], num_agents, axis=2)
        user_rates = user_rates.reshape(self.episode_length, batch_size, self.num_users)
        if self.use_cvar:
            cvar_values = np.repeat(self.cvar_values[:, :, None, :], num_agents, axis=2)
            cvar_values = cvar_values.reshape(self.episode_length, batch_size, self.num_users)
            cvar_betas = np.repeat(self.cvar_betas[:, :, None, :], num_agents, axis=2)
            cvar_betas = cvar_betas.reshape(self.episode_length, batch_size, 1)
        ...
        for start_ind in range(0, batch_size, num_envs_per_batch):
            ...
            for offset in range(num_envs_per_batch):
                ...
                user_rates_batch.append(user_rates[:, ind])
                if self.use_cvar:
                    cvar_values_batch.append(cvar_values[:, ind])
                    cvar_betas_batch.append(cvar_betas[:, ind])
            ...
            user_rates_batch = np.stack(user_rates_batch, 1)
            if self.use_cvar:
                cvar_values_batch = np.stack(cvar_values_batch, 1)
                cvar_betas_batch = np.stack(cvar_betas_batch, 1)
            ...
            user_rates_batch = _flatten(T, N, user_rates_batch)
            if self.use_cvar:
                cvar_values_batch = _flatten(T, N, cvar_values_batch)
                cvar_betas_batch = _flatten(T, N, cvar_betas_batch)
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        # user_rates 始终存在（无论 use_cvar）
        user_rates = np.repeat(self.user_rates[:, :, None, :], num_agents, axis=2)
        user_rates = _cast(user_rates)
        if self.use_cvar:
            cvar_values = np.repeat(self.cvar_values[:, :, None, :], num_agents, axis=2)
            cvar_values = _cast(cvar_values)
            cvar_betas = np.repeat(self.cvar_betas[:, :, None, :], num_agents, axis=2)
            cvar_betas = _cast(cvar_betas)
        ...
        for indices in sampler:
            ...
            for index in indices:
                ...
                user_rates_batch.append(user_rates[ind:ind + data_chunk_length])
                if self.use_cvar:
                    cvar_values_batch.append(cvar_values[ind:ind + data_chunk_length])
                    cvar_betas_batch.append(cvar_betas[ind:ind + data_chunk_length])
            ...
            user_rates_batch = np.stack(user_rates_batch, axis=1)
            if self.use_cvar:
                cvar_values_batch = np.stack(cvar_values_batch, axis=1)
                cvar_betas_batch = np.stack(cvar_betas_batch, axis=1)
            ...
            user_rates_batch = _flatten(L, N, user_rates_batch)
            if self.use_cvar:
                cvar_values_batch = _flatten(L, N, cvar_values_batch)
                cvar_betas_batch = _flatten(L, N, cvar_betas_batch)
